# Necessary sample size to reach desired power for a two-way between-subjects ANOVA using a publication bias and uncertainty correction procedure

`ss_buc_factorial_anova` returns the necessary per-cell sample size to
achieve a desired level of statistical power for a planned study testing
an omnibus main effect or interaction in a two-way fully
between-subjects ANOVA, based on information obtained from a previous
...
for a design with more than two factors, or for a
...
- effect:
  ...
  Effect of most interest to the planned study: main effect of A
  (`factor_A`), main effect of B (`factor_B`), or interaction
  (`interaction`). Case-insensitive shorthand is also accepted but not
  recommended in scripts: `"A"` (or `"a"`) for `factor_A`, `"B"` for
  `factor_B`, and `"AxB"` (or `"AB"`) for `interaction`.
...
The approach implemented in `ss_buc_factorial_anova` uses the observed
...
`ss_buc_factorial_anova` assumes that the planned study will have equal
...
equally divisible by the number of cells, the function calculates *n* by
...
#>  desired_power        0.8  
#> 
#> Design: Two-way between-subjects ANOVA (factor_B)
...
result$value[result$term == "necessary_n_per_cell"]   # per cell
#> [1] 659
