# Necessary sample size to reach desired power for a dependent (paired) t test (or standardized mean difference) using a publication bias and uncertainty correction procedure

`ss_buc_paired_t` returns the necessary sample size (the number of
pairs) to achieve a desired level of statistical power for a planned
study using a dependent *t* test, based on information obtained from a
...
`ss_buc_smd_paired` is an alias for `ss_buc_paired_t`: the two are the
same function. The paired *t* test and the standardized mean difference
of the difference scores describe the same comparison, so the same
planner serves both framings. Prefer the test-specific name
(`ss_buc_paired_t`) when the prior *t* was computed from raw
...
- N:
  ...
  Total sample size (the number of pairs) of the previous study.
...
number of pairs for the planned study, named for its unit
...
The approach implemented in `ss_buc_paired_t` uses the observed *t*
...
used to calculate the necessary sample size to achieve the desired level
of power in the planned study.
...
`t_observed = 3` plans. The sign of a paired *t* records the direction
the within-pair difference was taken, an arbitrary coding choice rather
than evidence. When planning a replication, confirm that the magnitude
...
If you are working from a standardized mean difference (Cohen's \\d_z\\)
of the difference scores rather than a *t* statistic, convert it before
calling: for a paired design \\t = d_z\sqrt{N}\\, where *N* is the
number of pairs.
