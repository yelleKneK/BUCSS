# Necessary sample size to reach desired power for an analysis of covariance using a publication bias and uncertainty correction procedure

`ss_buc_ancova` returns the necessary per-cell sample size to achieve a
desired level of statistical power for a planned study testing a group
effect (omnibus or contrast) while adjusting for one or more covariates,
...
- F_observed:
  ...
  Observed *F* value for the effect of interest from a previous study
  used to plan sample size for a planned study. For a
  single-degree-of-freedom contrast, this is the square of the
  contrast's *t*.
...
- cells:
  ...
  Number of cells (groups) in the between-subjects design.

- n_covariates:

  Number of covariates adjusted for in the analysis.

- df_numerator:

  Numerator degrees of freedom for the effect of interest. Defaults to
  `cells - 1`, the omnibus group effect; use 1 for a
...
estimated parameters. `ss_buc_ancova` takes the number of covariates
directly and carries them through to the planned study: the prior
study's error degrees of freedom are *N* minus the number of cells minus
the number of covariates, and the planned study's error degrees of
freedom are reduced by the same number of covariates. Supplying
`n_covariates = 0` reproduces the corresponding
...
(`df_numerator = cells - 1`). For a single-degree-of-freedom contrast
among the adjusted means, supply the contrast's *F* (the square of its
*t*) and set `df_numerator = 1`.
...
`ss_buc_ancova` assumes that the planned study will have equal *n*. If
the prior *N* is not equally divisible by the number of cells, the
per-cell size is rounded both up and down, the planned sample size is
computed under each, and the larger of the two is returned, with the
smaller of the two adjusted noncentrality parameters, to be
conservative.
...
[`ss_buc_factorial_anova_general`](https://yelleKneK.github.io/BUCSS/reference/ss_buc_factorial_anova_general.md),
which this function calls, and which takes the error degrees of freedom
directly.
