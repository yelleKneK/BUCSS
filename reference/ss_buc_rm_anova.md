# Necessary sample size to reach desired power for a one or two-way within-subjects ANOVA using a publication bias and uncertainty correction procedure

`ss_buc_rm_anova` returns the necessary total sample size to achieve a
desired level of statistical power for a planned study testing an
omnibus effect using a one or two-way fully within-subjects ANOVA, based
on information obtained from a previous study. The effect from the
...
- levels_B:
  ...
  Number of levels for Factor B, which is NULL if a single factor
  design.
...
The approach implemented in `ss_buc_rm_anova` uses the observed *F*
...
used to calculate the necessary total sample size to achieve the desired
level of power in the planned study.
...
`ss_buc_rm_anova` assumes sphericity for the within-subjects effects.
...
#>  desired_power 0.8  
#> 
#> Design: One or two-way within-subjects ANOVA (factor_B)
