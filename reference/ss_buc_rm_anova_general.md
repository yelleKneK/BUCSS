# Necessary sample size to reach desired power for a within-subjects ANOVA with any number of factors using a publication bias and uncertainty correction procedure

`ss_buc_rm_anova_general` returns the necessary total sample size to
achieve a desired level of statistical power for a planned study testing
any type of effect (omnibus, contrast) using a fully within-subjects
ANOVA with any number of factors, based on information obtained from a
...
The approach implemented in `ss_buc_rm_anova_general` uses the observed
...
`ss_buc_rm_anova_general` assumes sphericity for the within-subjects
effects.
...
#> Largest supportable assurance: .74
