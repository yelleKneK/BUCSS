...
#'   study noncentrality parameter). The design, the effect tested, the unit
#'   the sample size is counted in, the largest assurance this prior can
#'   support (the assurance ceiling), the planned test's degrees of freedom
#'   (\code{df_effect} and \code{df_error}), and the planning inputs travel on
#'   attributes; they are shown by \code{print.bucss_power}, and \code{tidy()}
#'   and \code{glance()} return them as columns.
