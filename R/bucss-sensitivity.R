...
# from and see how often the plan actually works.
#
# The design table below is the one piece of per-design knowledge this file
# needs: for each planner, which argument carries the prior statistic, what
# distribution that statistic follows, and the prior study's degrees of
# freedom. The degrees of freedom recorded here are the prior study's OWN,
# which is what a simulated prior study must be drawn from. They can differ
# from the degrees of freedom a planner uses internally: the between-subjects
# and split-plot planners round an unbalanced prior N to a balanced design in
# two directions and correct under each, which is a deliberate conservatism of
# the planner rather than a fact about the prior study.
#
# Every entry is pinned in tests/testthat/test-sensitivity.R against an
# independently written computation, and ss_buc_sensitivity() additionally
# checks at run time that rebuilding the planner call from the result object
# reproduces that object exactly.

.BUCSS_SENSITIVITY_SPECS <- list(
...
# names is .sensitivity_args()'s job.
.sensitivity_inputs <- function(object) {
...
.sensitivity_args <- function(inputs, spec, effect) {
  args <- inputs
...
ss_buc_sensitivity <- function(object, true_ncp, replications = 1000L,
  ...
  spec <- .BUCSS_SENSITIVITY_SPECS[[design]]
  if (is.null(spec))
    stop("No simulation is defined for the design '", design, "'.", call. = FALSE)

  inputs <- .sensitivity_inputs(object)
  ...
  args <- .sensitivity_args(inputs, spec, effect)
